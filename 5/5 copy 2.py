...
if __name__ == "__main__":
    ...
    for i in range (0,len(seeds_list),2):
        for j in range (0, 5):
            p_id += 1
            if j != 4:
                p = Process(target=get_min_location, args=(p_id, seeds_list[i] + int((seeds_list[i+1]/5)*j),(int(seeds_list[i+1]/5)), maps, return_dict))
            else:
                p =Process(target=get_min_location, args=(p_id, seeds_list[i] + int((seeds_list[i+1]/5)*j), (int(seeds_list[i+1]/5) + seeds_list[i+1]%5), maps, return_dict))
            jobs.append(p)
            p.start()
    for p in jobs:
        p.join()
